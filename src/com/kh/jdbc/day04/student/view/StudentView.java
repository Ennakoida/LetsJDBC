		...
		while(true) {
			...
			switch(choice) {
				case 1 :
					...
					// SELECT COUNT(*) FROM STUDENT_TBL -> int 로 가능
				...
				case 2 :
					...
					break;
				case 3 :
					// SELECT * FROM STUDENT_TBL WHERE STUDENT_NAME LIKE '일용자';
					String studentName = inputStdName();
					sList = controller.selectAllByName(studentName);
//					if(!sList.isEmpty()) {
					if(sList.size() > 0) { // 데이터가 존재함
						printAllStudents(sList);
					} else {
						displayError("데이터 조회에 실패하였습니다.");
					}
				...
				case 6 :
					...
					// DELETE FORM STUDENT_TBL WHERE STUDENT_ID = 'khuser01';
	...
	private String inputStdId(String category) {
		...
		System.out.print(category + "할 아이디 : ");
